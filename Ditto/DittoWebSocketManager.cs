...
namespace Verhaeg.IoT.Ditto
{
    public abstract class DittoWebSocketManager
    {
        ...
        {
            ArraySegment<byte> asb = new ArraySegment<byte>(Encoding.UTF8.GetBytes(data));
            await socket.SendAsync(asb, WebSocketMessageType.Text, true, CancellationToken.None).ConfigureAwait(false);
        }
